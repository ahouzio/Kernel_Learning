...
def plotRegression(X, Y,Y_clean=None, model=None, label='',  regressionLabel='Regression', 
            ax=None, bound=[[-1., 1.], [-0.8, 1.2]]):
    """ Plot the SVM separation, and margin """
    colors = ['blue']
    cmap = pltcolors.ListedColormap(colors)
    if ax is None:
    ...
    if model is not None:
        ...
        ax.plot(xx, Y_pred+model.b, color='grey', label=regressionLabel, linestyle='-')
        # Plot margin
        if model.type=='svr' and model.support is not None:
            ind = (Y-Y_pred > model.b + model.eta) + (Y-Y_pred < model.b-model.eta)
            ax.scatter(X[ind], Y[ind], label='Beyond the margin', s=80, facecolors='none', 
                edgecolors='grey', color='grey') 
            ax.scatter(model.support[:,0], model.support[:,1], label='Support', s=80, facecolors='none', edgecolors='r', color='r')
            print("Number of support vectors = %d" % (len(model.support)))
            ax.plot(xx, Y_pred+model.b+model.eta, color='grey', label='Tube +', linestyle='-.', alpha=0.8)
            ax.plot(xx, Y_pred+model.b-model.eta, color='grey', label='Tube -', linestyle='--', alpha=0.8)
